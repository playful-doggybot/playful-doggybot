...
class TerrainPerlin:
    def __init__(self, cfg, num_envs):
        ...
        self.xSize = cfg.terrain_length * cfg.num_rows + cfg.border_size * 2 # int(cfg.horizontal_scale * cfg.tot_cols)
        self.ySize = cfg.terrain_width * cfg.num_cols + cfg.border_size * 2 # int(cfg.horizontal_scale * cfg.tot_rows)
        ...
        x, y = self.heightsamples.shape
        median_height = np.median(self.heightsamples)
        # i, j = 0, 0
        block_size = 30
        for i in range(int((x - block_size)/block_size)):
            for j in range(int((y-block_size)/block_size)):
                if (i % 2 == 0 and j % 2 ==0) or (i % 2 ==1 and j % 2 == 1):
                    self.heightsamples[i*block_size:i*block_size+block_size, j*block_size:j*block_size+block_size] = median_height
            

        print(self.heightsamples)
    ...
    def add_terrain_to_sim(self, gym, sim, device= "cpu"):
        ...
        for row_idx in range(self.cfg.num_rows):
            for col_idx in range(self.cfg.num_cols):
                origin_x = (row_idx + 0.5) * self.env_length + self.cfg.border_size
                origin_y = (col_idx + 0.5) * self.env_width + self.cfg.border_size
    ...
    def in_terrain_range(self, pos):
        ...
        return torch.logical_and(
            pos[..., :2] >= self.cfg.border_size,
            pos[..., :2] < torch.tensor([self.xSize - self.cfg.border_size, self.ySize - self.cfg.border_size], device= self.device),
        ).all(dim= -1)
